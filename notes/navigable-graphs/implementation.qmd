...
## Setup
**Goal:** Construct a graph $G = (V, E)$ with the fewest number of edges such that $G$ is _navigable_ for some distance metric $d$. $G$ is navigable iff for every $s,t \in V$, pair, there exists a directed edge $(s, k) \in E$ s.t. $d(k, t) < d(s,t)$. 
...
print(latex_matrix)
```
Observe the entries of each row: there are duplicate entries since multiple points can be equidistant from a given point. **I'm still not exactly sure on how to do tie-breaking**.

To construct the sets, we will compare each column of the permutation matrix against every other column, yielding the following sets. Here, $S_{u}$ corresponds to the matrix whose columns form the set cover instance for point $u$.
```{python}
#| output: asis
...
I will now walk-through the greedy set cover for point $C$. Initialize the uncovered vector $\mathbf u = [1, 1, 0, 1, 1]^\top$.
...
## Polytope
$x_t = \alpha_t 1_{S_t} + (1- \alpha_t) x_{t+1}$